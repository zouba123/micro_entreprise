...
class SaleInvoiceReturn {
  ...
  DateTime createdAt;
  DateTime modifiedAt;
  List<SaleInvoiceReturnDetail> invoiceReturnDetailList = [];
  List<SaleInvoiceReturnTax> invoiceReturnTaxList = [];
  ...
  List<SaleInvoiceReturn> childList = [];
  bool isSelect = false; // use during invoice selection in return invoice
//  List<TextEditingController> cList = []; // use during sales rturn

  SaleInvoiceReturn(this.id, this.saleInvoiceId, this.transactionGroupId, this.invoiceNumber, this.accountId, this.netAmount, this.discount, this.grossAmount, this.finalTax, this.invoiceDate, this.isRefundTax, this.status, this.account);

  // SaleInvoiceReturn();

  SaleInvoiceReturn.fromMap(Map<String, dynamic> map) {
    id = map['id'];
