...
import 'package:accounting_app/reports/productGeneralReportFilterScreen.dart';
import 'package:accounting_app/screens/dashboardScreen.dart';
import 'package:material_design_icons_flutter/material_design_icons_flutter.dart';

class ProductGeneralReport extends BaseRoute {
  final ProductGeneralReportFilterModel filter;
  ProductGeneralReport({@required a, @required o, @required this.filter}) : super(a: a, o: o, r: 'ProductGeneralReport');
  @override
  _ProductGeneralReportState createState() => _ProductGeneralReportState(this.filter);
}

class _ProductGeneralReportState extends BaseRouteState {
  final ProductGeneralReportFilterModel filter;
  bool _isDataLoaded = false;
  List<ProductGeneralReportModel> _productGeneralReportList = [];
  ProductGeneralReportFilterModel _productGeneralReportFilter =  ProductGeneralReportFilterModel();
  var _html;
  ...
  _ProductGeneralReportState(this.filter) : super();
                ...
                IconButton(
                    ...
                    onPressed: () async {
                      Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                              builder: (context) => ProductGeneralReportFilterScreen(
                                    a: widget.analytics,
                                    o: widget.observer,
                                    appliedFilter: _productGeneralReportFilter,
                                  )));
                    }),
              ],
            ),
            body: (_isDataLoaded)
                ? (_productGeneralReportList != null)
                    ? Stack(children: [
                        SizedBox(
  ...
  Future _init() async {
    try {
      if (filter != null) {
        _productGeneralReportFilter = filter;
      }
      ...
      print('Exception - ProductGeneralReport.dart - _init(): ' + e.toString());
    }
  }

  Future _getData() async {
    try {
      _productGeneralReportList = await dbHelper.getProductGeneralReportData(_productGeneralReportFilter);
    } catch (e) {
      print('Exception - ProductGeneralReport.dart - _getData(): ' + e.toString());
    }
  }
...
<head>
  ...
  """;
      _html += """ <h1 align=center><font size=6>${global.appLocaleValues['pro_gen_rep']}</font></h1> <br>
    ...
    <th>${global.appLocaleValues['hsn_code']}</th>""";
      ...
      if (_productGeneralReportFilter.showProductDescription) {
        _html += """
     <th>Description</th>
   """;
      }
   ...
   """;
      ...
      for (int i = 0; i < _productGeneralReportList.length; i++) {
        _html += """ <tr>
     <td>${i + 1}</td>
    <td>${br.getSystemFlagValue(global.systemFlagNameList.productCodePrefix)}${'0' * (int.parse(br.getSystemFlagValue(global.systemFlagNameList.productCodeMaxLength)) - (br.getSystemFlagValue(global.systemFlagNameList.productCodePrefix).length + _productGeneralReportList[i].productCode.toString().length))}${_productGeneralReportList[i].productCode}</td>
     <td>${_productGeneralReportList[i].productName}</td>
     <td>${_productGeneralReportList[i].productTypeName}</td>
     <td>${_productGeneralReportList[i].productHsnCode}</td>
    """;

        if (_productGeneralReportFilter.showProductDescription) {
          _html += """ <td>${_productGeneralReportList[i].productDescription}</td>
     """;
        }

        _html += """ <td>${global.currency.symbol} ${_productGeneralReportList[i].productPrice.toStringAsFixed(int.parse(br.getSystemFlagValue(global.systemFlagNameList.decimalPlaces)))}</td>
     </tr>""";
      }
  ...
  <footer>
    ...
    justify-content: center;">Generated on: ${DateFormat(br.getSystemFlagValue(global.systemFlagNameList.dateFormat)).format(DateTime.now())} ${DateFormat.jm().format(DateTime.now())}</span>
      ...
      print('Exception - ProductGeneralReport.dart - _buildHtml(): ' + e.toString());
    }
  }
      ...
      child: RawMaterialButton(
        ...
        onPressed: () async {
          ...
          // }
            ...
            global.isAppOperation = true;
            await br.shareReport(_html, 'ProductGeneralReport.pdf');
          }
